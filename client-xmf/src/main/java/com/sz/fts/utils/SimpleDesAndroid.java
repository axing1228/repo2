...
public class SimpleDesAndroid {
    ...
    public static final String strDefaultKey = "t2b4h6y4l5";
     ...
     * @return 加密后的字符串(密文)
     * @throws Exception 加密时, direction的值只能是DataDirection.TO_MOBILE 或
     *                   DataDirection.TO_SERVER, 否则会抛出LawlessDataDirectionException
     */
    public static String encrypt(String text, DataDirection direction) throws Exception {
        if (direction == DataDirection.FROM_MOBILE || direction == DataDirection.FROM_SERVER) {
        }
     ...
     * 解密字符串
     *
     * @param text 需解密的字符串(密文)
     * @return 解密后的字符串(明文)
     * @throws Exception 解密时, direction的值只能是DataDirection.FROM_MOBILE 或
     *                   DataDirection.FROM_SERVER, 否则会抛出LawlessDataDirectionException
     */
    public static String decrypt(String text, DataDirection direction) throws Exception {
        if (direction == DataDirection.TO_MOBILE || direction == DataDirection.TO_SERVER) {
        }
